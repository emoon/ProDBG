...
char* nvgCPToUTF8(int cp, char* str)
{
	...
	switch (n) 
	{
		case 6: str[5] = (char)(0x80 | (cp & 0x3f)); cp = cp >> 6; cp |= 0x4000000;
		case 5: str[4] = (char)(0x80 | (cp & 0x3f)); cp = cp >> 6; cp |= 0x200000;
		case 4: str[3] = (char)(0x80 | (cp & 0x3f)); cp = cp >> 6; cp |= 0x10000;
		case 3: str[2] = (char)(0x80 | (cp & 0x3f)); cp = cp >> 6; cp |= 0x800;
		case 2: str[1] = (char)(0x80 | (cp & 0x3f)); cp = cp >> 6; cp |= 0xc0;
