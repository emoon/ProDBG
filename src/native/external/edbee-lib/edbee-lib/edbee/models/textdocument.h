...
public:
 ...
 // Methods directly forwarded to the  textbuffer
    ...
    Q_SIGNAL void textAboutToBeChanged( edbee::TextBufferChange change );
    Q_SIGNAL void textChanged( edbee::TextBufferChange change );

    /// This signal is emitted if the persisted state is changed
    Q_SIGNAL void persistedChanged(bool persisted);

    /// This signal is emitted if the grammar has been changed
    Q_SIGNAL void languageGrammarChanged();

    /// this signal is emitted if the scoped range has been changed
    Q_SIGNAL void lastScopedOffsetChanged( int previousOffset, int lastScopedOffset );
